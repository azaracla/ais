...
def fetch_raw_files_window():
   """Récupère récursivement les URIs S3 des dossiers bruts pour AUJOURD'HUI uniquement"""
   s3 = boto3.client(
       's3',
...
def run_consolidation():
   ...
   print(f"🧹 Début de la consolidation TEST pour AUJOURD'HUI : {today}")
           ...
           con.execute(f"""
               ...
               SELECT
                   md5(concat_ws('|', 
                       ...
                       CASE WHEN COALESCE(CAST(NULLIF(regexp_extract(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):'), '') AS INTEGER), 0) > 23
                            THEN CAST(regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00'), ' (\\d{1,2}):', ' 00:') AS TIMESTAMPTZ)
                            ELSE CASE WHEN json_extract_string(metadata, '$.time_utc') LIKE '% 2_:%' THEN CAST(regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):', ' 00:'), ' \\+0000 UTC$', '+00:00') AS TIMESTAMPTZ) ELSE CAST(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00') AS TIMESTAMPTZ) END
                       END,
                   ...
                   COALESCE(
                       ...
                       CAST(json_extract_string(message, '$.LongRangeAisBroadcast.Sog') AS DOUBLE)
                   ) AS sog,
                   COALESCE(
                       ...
                       CAST(json_extract_string(message, '$.LongRangeAisBroadcast.Cog') AS DOUBLE)
                   ) AS cog,
                   COALESCE(
                       ...
                       CAST(json_extract_string(message, '$.LongRangeAisBroadcast.TrueHeading') AS INTEGER)
                   ) AS true_heading,
                   COALESCE(
                       CAST(json_extract_string(message, '$.PositionReport.NavigationalStatus') AS INTEGER),
                       CAST(json_extract_string(message, '$.LongRangeAisBroadcast.NavigationalStatus') AS INTEGER)
               ...
               WHERE message_type IN ('PositionReport', 'ExtendedClassBPositionReport', 'StandardClassBPositionReport', 'LongRangeAisBroadcast')
                 AND CAST(CASE WHEN COALESCE(CAST(NULLIF(regexp_extract(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):') AS INTEGER) > 23
                              THEN regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00'), ' (\\d{1,2}):', ' 00:')
                              ELSE regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00')
                          END AS TIMESTAMPTZ) BETWEEN '{today} {h_start}' AND '{today} {h_end}'
           ...
           WITH union_static AS (
               SELECT
                   ...
                   CASE WHEN CAST(json_extract_string(message, '$.ShipStaticData.Eta.Month') AS INTEGER) BETWEEN 1 AND 12 
                        ...
                        MAKE_TIMESTAMP(
                           COALESCE(CAST(json_extract_string(message, '$.ShipStaticData.Eta.Year') AS INTEGER), 2024), 
                           ...
                           LEAST(COALESCE(CAST(json_extract_string(message, '$.ShipStaticData.Eta.Hour') AS INTEGER), 0), 23), 
                           LEAST(COALESCE(CAST(json_extract_string(message, '$.ShipStaticData.Eta.Minute') AS INTEGER), 0), 59), 
                           0
                        )
                   ...
                   CASE WHEN COALESCE(CAST(NULLIF(regexp_extract(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):') AS INTEGER) > 23
                        THEN CAST(regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00'), ' (\\d{1,2}):', ' 00:') AS TIMESTAMPTZ)
                        ELSE CASE WHEN json_extract_string(metadata, '$.time_utc') LIKE '% 2_:%' THEN CAST(regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):', ' 00:'), ' \\+0000 UTC$', '+00:00') AS TIMESTAMPTZ) ELSE CAST(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00') AS TIMESTAMPTZ) END
                   END AS last_updated,
                   listener_id AS source_listener
               ...
               WHERE message_type IN ('ShipStaticData', 'StaticDataReport')
                 AND CAST(CASE WHEN COALESCE(CAST(NULLIF(regexp_extract(json_extract_string(metadata, '$.time_utc'), ' (\\d{1,2}):') AS INTEGER) > 23
                              THEN regexp_replace(regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00'), ' (\\d{1,2}):', ' 00:')
                              ELSE regexp_replace(json_extract_string(metadata, '$.time_utc'), ' \\+0000 UTC$', '+00:00')
                          END AS TIMESTAMPTZ) BETWEEN '{today} 00:00:00' AND '{today} 23:59:59'
           )
